...
def processStatisticalInformation(field, op, op2, ft_0, ft_all, rl_0, rl_all, individual_output):
    if DBG_OPTIONS.IngCompResult:
        return processStatisticalInformationV2(field, op, op2, ft_0, ft_all, rl_0, rl_all, individual_output)
    else:
        return processStatisticalInformationV1(field, op, op2, ft_0, ft_all, rl_0, rl_all, individual_output)


def processStatisticalInformationV1(field, op, op2, ft_0, ft_all, rl_0, rl_all, individual_output):
    '''
...
if DBG_OPTIONS.IngCompResult:
    def __getStaticInfo(op, df, field):
        if field=='':
            pass
        else:
            info = op(df[field])
...
def processStatisticalInformationV2(field, op, op2, ft_0, ft_all, rl_0, rl_all, individual_output):
    '''
    process statistics info to calculate improvement rate
...
def compareResult(args, target_tl_obj, ft_output, rl_output, model_num, passed_res_comp_skip = -1):
    ...
    return  compareResultV3(args, target_tl_obj, ft_output, rl_output, model_num, passed_res_comp_skip)
    #return  compareResultV1(args, target_tl_obj, ft_output, rl_output, model_num, passed_res_comp_skip)

def compareResultV1(args, target_tl_obj, ft_output, rl_output, model_num, passed_res_comp_skip = -1):
    '''
    compare two result files and calculate improvement rate
...
# def compareResultV2(args, target_tl_obj, ft_output, rl_output, model_num, passed_res_comp_skip = -1):
#     '''
...
#
#     ##
#     ## process statistical info about entire target intersections
#     # individual_output = pd.DataFrame({'name': ['total']})
#     individual_output = pd.DataFrame({'name': ['total'], 'SA':['all target']})
#
#
#     ##-- construct EDGE info about entire target intersection
#     for tl in target_tl_obj:
#         in_edge_list = np.append(in_edge_list, target_tl_obj[tl]['in_edge_list'])
#         in_edge_list_0 = np.append(in_edge_list_0, target_tl_obj[tl]['in_edge_list_0'])
#         # if DBG_OPTIONS.PrintResultCompare:
...
def compareResultV3(args, target_tl_obj, ft_output, rl_output, model_num, passed_res_comp_skip = -1):
    '''
    compare two result files and calculate improvement rate for each intersection, each SA and overall
