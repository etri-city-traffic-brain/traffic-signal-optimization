...
PORT=4001 # 3001 # 1737  # 2727
TB_PORT=8001 # 7001 # 6163 #6006
...
echo "nohup python DistCtrlDaemon.py --port $PORT --map $MAP --target-TL "$TARGET" --num-of-learning-daemon $NUM_DAEMON --state $STATE --action $ACTION  --reward-func $REWARD_FUNC --validation-criteria $IMP_GOAL --epoch $EPOCH --model-save-period $MODEL_SAVE_PERIOD --cumulative-training $CUMULATIVE_TRAINING --num-of-optimal-model-candidate $NUM_OPT_MODEL_CANDIDATE --model-store-root-path $MODEL_STORE_PATH/$RESULT_DIR/${ACTION}_${REWARD_FUNC}_${EXP_OPTION} --copy-simulation-output $COPY_SIMULATION_OUTPUT > out.ctrl 2>&1 &"
