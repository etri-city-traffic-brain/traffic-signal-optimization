...
from TSOConstants_off_ppo import _REWARD_GATHER_UNIT_
...
class SaltRewardMgmtV3:
    ...
    def __constructDictionaryForRewardCalculation(self):
        ...
        # dic_calc_reward['wt_SBV'] = ['sum', -1]
        # dic_calc_reward['wt_SBV_max'] = ['max', -1]
        dic_calc_reward['dt'] = ['sum', -1]
        dic_calc_reward['sdt'] = ['sum', -1]
    ...
    def __getRewardInfo(self, reward_info, link_list_0, lane_list_0, sim_steps):
        ...
        if self.reward_func == 'pn':
            for l in link_list_0:
                reward_info = np.append(reward_info, libsalt.link.getSumPassed(l)/5000)
        ...
        #                                 libsalt.link.getCurrentAverageWaitingTimeBaseVehicle(l, sim_steps) / 1000)
            ...
            for l in link_list_0:
                reward_info = np.append(reward_info,
                                        libsalt.link.getSumTravelTime(l) / (len(link_list_0) * self.reward_info_collection_cycle)/200)
        
        elif self.reward_func == 'dt':
            for l in link_list_0:
                reward_info = np.append(reward_info, libsalt.link.getAverageDensity(l)/40)
            #for lane in lane_list_0:
            #    reward_info = np.append(reward_info, libsalt.lane.getAverageDensity(lane))
        elif self.reward_func == 'sdt':
            for l in link_list_0:
                reward_info = np.append(reward_info, libsalt.link.getAverageDensity(l)/120 * sum([l in x for x in lane_list_0]))
            #for lane in lane_list_0:
            #    reward_info = np.append(reward_info, libsalt.lane.getAverageDensity(lane))
