...
from env.off_ppo.SaltEnvUtil import appendTsoOutputInfo, initTsoOutputInfo

if DBG_OPTIONS.RichActionOutput:
    from env.off_ppo.SaltEnvUtil import appendTsoOutputInfoSignal
    from env.off_ppo.SaltEnvUtil import replaceTsoOutputInfoOffset, replaceTsoOutputInfoDuration
...
class SaltSappoEnvV3(gym.Env):
    ...
    def __init__(self, args):  # equal SALT_SAPPO_offset_single.__init__()
        ...
        if 1:
            ...
            #print('ID List', len(self.tl_id_list))
            #for tl_id in self.tl_id_list:
                #print(tl_id, ',', self.tl_obj[tl_id]['action_space'], ',', self.tl_obj[tl_id]['minDur'], ',', self.tl_obj[tl_id]['maxDur'])
                #print(self.tl_obj[tl_id]['minDur'])
                #print(self.tl_obj[tl_id]['maxDur'])
            ...
            for sa_name in self.sa_name_list:
                ...
                self.sa_obj[sa_name]['action_space'] = spaces.Box(low=np.array(self.sa_obj[sa_name]['action_min']),
                                                                        ...
                                                                        dtype=np.float32) #dtype=np.int32)
