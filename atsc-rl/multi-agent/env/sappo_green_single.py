...
class SALT_SAPPO_green_single(gym.Env):
            ...
            if self.simulationSteps % (self.sa_obj[sa]['cycle_list'][0]*self.control_cycle) == 0:
                ...
                for tlid_i in range(len(tlid_list)):
                    ...
                    for _i in range(len(green_idx)):
                        gi = green_idx[_i]
                        _m = list(mpv[gi])
                        _m[0] = currDur[gi] + int(action[_i]) * self.args.addTime
                        mpv[gi] = tuple(_m)
                    ...
                    __phase_list_include_y = [x[0] for x in libsalt.trafficsignal.getCurrentTLSScheduleByNodeID(tlid).myPhaseVector]
